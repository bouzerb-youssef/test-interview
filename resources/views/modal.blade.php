<div class="modal h-screen w-full fixed top-0 left-0 flex justify-center items-center Z-50 hidden    ">
    
  <form action="{{ route('contacts.store') }}" method="POST" id="contact-form">
    @csrf
        <div class=" mx-auto mt-10 p-6 bg-white  rounded-t-lg shadow-md">
            <h2 class="text-2xl font-normal mb-4">Détail du contact</h2>
            
              <!-- Row 1: Prenom and Nom -->
              <div class="flex mb-4">
              ...
              <div class="mb-4">
                ...
                <textarea id="addresse" name="addresse" class="w-full border border-[#848d98] shadow-md  rounded-md px-3 py-2 focus:outline-none focus:ring focus:border-blue-300" ></textarea>
              </div>
        
             
              <div class="flex mb-4">
                <div class="mr-2 w-1/3">
                  <label for="codepostal" class="block text-[#848d98]">Code Postal</label>
          ...
          <div class="flex justify-end gap-1 bg-[#eff5f5] mt-2">
            <button type="button" class="px-4 py-2  text-[#45505e] rounded-md border close-modal">Annuler</button>
            <button type="submit" class="px-4 py-2 bg-[#0ebccb] text-white rounded-md border border-[black]">Submit</button>
          </div>
